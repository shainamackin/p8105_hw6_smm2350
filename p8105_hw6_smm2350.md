...
Now, I’ll use 5000 boostrap samples to produce estimates for r̂^2 and
log(β̂0∗β̂1).
...
knitr::kable(weather_bootstrap_r2)
```
...
weather_bootstrap_log = weather_df %>%
  ...
  group_by(.id) %>%
  summarize(log_b0b1 = prod(log_est)) %>%
  summarize(
    ci_lower = quantile(log_b0b1, 0.025), 
    ci_upper = quantile(log_b0b1, 0.975))

knitr::kable(weather_bootstrap_log)
```
