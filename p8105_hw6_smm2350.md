...
``` r
proposed_fit = MASS::stepAIC(saturated_fit, direction = "backward", trace = FALSE)

proposed_fit%>%broom::tidy() %>%knitr::kable()
```
...
My `proposed_fit` thus includes `babysex`, `bhead`, `blength`, `delwt`,
...
``` r
bwt_resid = add_residuals(bwt_df, proposed_fit) 

bwt_pred = add_predictions(bwt_df, proposed_fit)
...
It’s time to compare my model to two others using cross validation:
...
cv_df2 = cv_df %>% 
  mutate(
    proposed_fit  = map(train, ~lm(bwt ~ babysex + bhead + blength + delwt + fincome + gaweeks + mheight + mrace + parity + ppwt + smoken, data =.x)),
  ...
  mutate(
    rmse_proposed = map2_dbl(proposed_fit, test, ~rmse(model = .x, data = .y)),
